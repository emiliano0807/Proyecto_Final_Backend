...
module.exports = {
  ...
  getById: async (req, res) => {
    ...
    try {
      ...
      const result = await pool.request()
        .input('id', id)
        .query('SELECT * FROM usuariosLaVega WHERE id_Usuarios = @id');
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      await pool.request()
        ...
        .query(`
          INSERT INTO usuariosLaVega (id_Usuarios, nombre, Apellido_1, Apellido_2)
  ...
  remove: async (req, res) => {
    ...
    try {
      ...
      const result = await pool.request()
        .input('id', id)
        .query('DELETE FROM usuariosLaVega WHERE id_Usuarios = @id');
      if (result.rowsAffected[0] === 0) return res.status(404).send('Usuario no encontrado');
